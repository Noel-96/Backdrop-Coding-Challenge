...
final class CatListViewModel: ObservableObject{
    ...
    @Published var catApiList = [CatModel]()
    ...
    func fetchList() {
        self.networkRequest.fetchListSignal()
            ...
            // specify that we want to receive publisher on main thread scheduler (for UI ops)
            .mapError { APIServiceError.mappedFromRawError($0) }
            // map any error signal
            .sink(receiveCompletion: { [weak self] (completion) in
                // completion will be trigger eventually (at the end of signal chain)
                defer { self?.isLoading = false }
                // finalize `isLoading` state
